...
class PenjualanDetailController extends Controller
{
    ...
    public function data($id)
    {
        ...
        foreach ($detail as $item) {
            ...
            $row['jumlah'] = '<input type="number" class="form-control form-control-sm quantity" data-id="'.$item->id_penjualan_detail.'" value="'.
            $item->jumlah.'" step="0.1">';
            $row['diskon'] = $item->diskon. '%';
            $row['subtotal'] = format_uang($item->subtotal);
            $row['aksi']        = '<div class="btn-group">
                                    <button onclick="deleteData(`'. route('transaksi.destroy', $item->id_penjualan_detail) .'`)" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i>Hapus</button>
                                </div>';
            $data[] = $row;
